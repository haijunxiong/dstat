...
def getInstalledSoftFromRegistry():
    softs = set()
...
def getOs():
    ...
    os["name"] = getOsName()
    os["partition"] = getPartition()

    softs = getInstalledSoftFromWmi()

    if len(softs) <= 0:
        softs = getInstalledSoftFromRegistry()
    ...
    if len(webserver) <= 0:
        webserver = getWebServerInfoFromIIS6()

    os["webserver"] = webserver

    os["network"] =  getNetwork()
...
def getBaseInfo():
    info={}
    hardware = {}         
    hardware["cpu"] = getCpus()
    hardware["memory"] = getMemory()
    hardware["disk"] = getDrives()
