...
try:
    ...
    import types, getpass, glob, linecache
...
class Options:
    def __init__(self, args):
        ...
        if not self.plugins:            
            if win:
                print 'You did not select any stats, using -counter by default.'
                self.plugins = ['counter']
            else:    
...
#            raise Exception, 'File %s does not exist' % self.fd
    ...
    def showcsv(self):
        ...
        for i, name in enumerate(self.vars):
            ...
            elif isinstance(self.val[name], types.DictType):
                line = line + json.dumps(self.val[name])                
            else:
...
class dstat_counter(dstat):
    ...
    def extract(self):
        ...
        for s in set1:
            if s == 'total' :  continue
            if(set2.has_key(s)):        
                #process.append( (s,(sum(set2[s][0:2]) - sum(set1[s][0:2]) )* 100 / totaltime, set2[s][2],set2[s][3],set2[s][4],set2[s][5]) )
                proc = {}
                proc["pid"] = s
...
def listplugins():
    ...
    for path in pluginpath:
        for filename in glob.glob(os.path.join(path,'dstat_*.py')):
            plugins.append(remod.match(os.path.basename(filename)).groups()[0].replace('_', '-'))
    plugins.sort()
    return plugins
...
#       print 'Background is unknown, assuming dark.'
    ...
    if op.output:
        if os.path.exists(op.output):
            ...
            outputfile.write('"Tstat %s CSV output"\n' % VERSION)
